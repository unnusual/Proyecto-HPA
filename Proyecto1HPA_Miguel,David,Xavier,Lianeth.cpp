...
void printINF(char nombres[][35], char cedulas[][15], int aservicio[], float salario_bruto[], float salario_neto[], float aumentos[], int n)
{
    float totalBruto = 0.0, totalAumento = 0.0, totalNeto = 0.0, x[20];
	int a, b; 
	char mayor[35], menor[35];
    
    printf("\n\t\t\tBANCO PANAMEÑO DE PRODUCCION\n");
    printf("\t\t\t LISTADO DE AUMENTO SALARIAL\n\n");
    printf("%-20s %-15s %-5s %-15s %-10s %-15s\n", "NOMBRE", "CEDULA", "AÑOS", "SALARIO BRUTO", "AUMENTO", "SALARIO NETO");
    for (int i = 0; i < n; i++) 
	{
		totalBruto += salario_bruto[i];
		totalAumento += calcularAumento(salario_bruto[i],aservicio[i]);
		x[i] = calcularSalNet(salario_bruto[i],aservicio[i]);
        totalNeto += x[i];
        printf("%-20s %-15s %-5d $%-14.2f $%-9.2f $%-14.2f\n", nombres[i], cedulas[i], aservicio[i], salario_bruto[i], calcularAumento(salario_bruto[i],aservicio[i]), x[i]);
    
    }
    a= encontrarMayor(x,n);
	b= encontrarMenor(x,n);
    printf("\n%-42s $%-14.2f $%-9.2f $%-10.2f\n\n", "TOTALES:", totalBruto,totalAumento, totalNeto);
    printf("EMPLEADO DE MAYOR SUELDO: %-16s con $%.2f\n" , nombres[a],x[a] );
    printf("EMPLEADO DE MENOR SUELDO: %-16s con $%.2f\n", nombres[b],x[b] );
}
